...
using static CarContollingScripts.CarController;
...
namespace CarContollingScripts
{
    public class CarController : MonoBehaviour
    {
        ...
        public float stabilizationFactor = 0.5f; // Adjust this value to fine-tune stabilization

        public GameObject steeringWheelModel;
        public float maxSteeringWheelAngle = 180.0f; // Максимальный угол поворота руля
        ...
        void Brake()
        {
            if (isBraking)
            {
                for (int i = 0; i < wheels.Count; i++)
                {
                    ...
                    //wheels[i].wheelCollider.brakeTorque = 300 * brakeAcceleration;
                }
        ...
        void AnimateSteeringWheel()
        {
            ...
            {
                float steeringAngle = -steerInput * maxSteeringWheelAngle;
                steeringWheelModel.transform.localRotation = Quaternion.Euler(0, 0, steeringAngle);
            }
        }
        ...
        void ApplyStabilization()
        {
            ...
            {
                // Apply a force to counter lateral velocity (sideways drift)
                Vector3 localVelocity = transform.InverseTransformDirection(carRb.velocity);
                Vector3 lateralForce = -transform.right * (localVelocity.x * stabilizationFactor);

                // Apply the stabilization force, dampening the car's lateral movement
